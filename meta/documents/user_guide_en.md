...
The **Disable the Ceres Product schema on item pages** option prevents a second Product node generated by Ceres. It is enabled by default and only takes effect together with **Render Product/ProductGroup/Offer JSON-LD server-side**. Make sure that the **Product/ProductGroup/Offer JSON-LD server-side** data provider is assigned to the single-item layout.

### Exact shipping-profile prices in version 5.0.42

Use **“Shipping-profile fallback prices”** for mappings such as `6=6.90; 9=59.00; 12=89.00`. The left side is the PlentyONE shipping profile ID and the right side is the shipping price. Separate mappings with semicolons or line breaks. PlentyONE calculated default shipping costs always keep priority.

### Shipping and video in version 5.0.41

Enable the shipping fallback only after verifying parcel and freight prices for the respective plugin set. PlentyONE calculated default shipping costs always take precedence. If absent, the plugin selects freight for a matching shipping profile ID or at the configured gross-weight threshold; otherwise it selects the parcel amount. A blank amount produces no shipping markup.

The two video property settings accept multiple comma-separated IDs. The first valid YouTube ID and upload date are used. No `VideoObject` is emitted unless both values are valid.
